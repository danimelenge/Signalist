...
/// Vista principal del conversor de Cifrado César.
struct CaesarView: View {

    // MARK: - Properties

    @ObservedObject var viewModel: CaesarViewModel
    ...
    /// Encabezado principal del conversor César.
    private var header: some View {
        VStack(spacing: 6) {
            ...
            Text("Cifra texto desplazando letras del alfabeto")
                .font(.subheadline)
    ...
    private var modePicker: some View {
        Picker("Modo", selection: $viewModel.mode.animation(.snappy)) {
            ForEach(CaesarConversionMode.allCases) { option in
    ...
    /// Control para ajustar el desplazamiento del cifrado (1-25).
    private var shiftControl: some View {
        VStack(alignment: .leading, spacing: 10) {

            HStack {
                Label("Desplazamiento", systemImage: "arrow.left.and.right")
                    .font(.headline)

                Spacer()

                Text("\(Int(viewModel.shift))")
            ...
            Slider(value: $viewModel.shift, in: 1...25, step: 1)
                .tint(.blue)
        }
        .padding(16)
        ...
        // NOTE:
        // El desplazamiento se comparte entre ambos modos: el mismo valor
        // se usa para cifrar y para descifrar, ya que descifrar es solo
        // aplicar el desplazamiento inverso.
    }

    // MARK: - Input Card

    /// Tarjeta donde el usuario escribe el texto o el cifrado César.
    private var inputCard: some View {
        VStack(alignment: .leading, spacing: 10) {

            Label(
                viewModel.mode == .textToCaesar ?
                "Texto de entrada" :
                "César de entrada",
